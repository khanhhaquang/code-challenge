...
const FormFieldWrapper = ({ children }: { children: ReactNode }) => {
	return <div className='flex gap-2 w-full mb-4 items-end rounded-lg bg-secondary p-2 relative'>{children}</div>;
};
					...
					<form className='flex flex-col' onSubmit={form.handleSubmit(handleSubmit)}>
						<FormFieldWrapper>
							...
							<FormField
								...
								name='fromToken'
								render={({ field }) => (
									<FormItem className='absolute top-0 right-2'>
										<Select onValueChange={field.onChange} value={field.value}>
											...
											<SelectContent className='z-10 bg-white cursor-pointer h-60'>
												{filteredPrices.map((p) => (
													<SelectItem value={p.currency} key={p.currency} className='cursor-pointer'>
						...
						<Button
							type='button'
							variant='outline'
							className='mx-auto w-fit h-12 outline-2 outline-white'
						...
						<Button className='mt-4' variant='outline' type='submit' disabled={!form.formState.isValid || isSubmitting}>
